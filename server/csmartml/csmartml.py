...
class CSmartML:
    def __init__(self, filename, population, time_budget, publish, meta_cvi=False, algorithm=None, cvi=None, dataset=None, result="multi"):
        ...
        if dataset is None:
            self.data = pd.read_csv(
                "./csmartml/datasets/{}.csv".format(filename)).iloc[:, :-1]
        else:
            self.data = dataset
        ...
        if meta_cvi:
            ...
            self.cvi = Meta_CVI(filename, "distance",
                                dataset).search(self.algorithm)
        ...
        fitness_weights = (np.float(self.cvi[0][1]), np.float(
            self.cvi[1][1]), np.float(self.cvi[2][1]))
        ...
        publish(
            {"label": "{} hyper-partitions generated...".format(len(hp_set))}, "message")
        ...
        for partition in hp_set:
            ...
            toolbox.register("individual", tools.initRepeat,
                             creator.Individual, mx.gen_population, n=1)
    ...
    def search(self, publish):
        ...
        if len(self.partitions) > 1:
            for key, value in self.partitions.items():
                ...
                if len(value) > 1:
                    func_dict['ea_custom-' + key] = [key, self.time,
                                                     population, self.toolbox[key], 0.7, 0.3, publish]
                else:
                    func_dict['random_search-' + key] = [key, self.time,
                                                         self.pop_size, population, self.toolbox[key], publish]
    ...
    def fitness_function(self, individual):
        ...
        try:
            ...
            validate = Validation(np.asmatrix(self.data).astype(
                np.float), np.asarray(self.data), labels)
            metric_values = validate.run_list(
                [self.cvi[0][0], self.cvi[1][0], self.cvi[2][0]])
...
def random_search(pid, t, s, population, toolbox, publish, res):
    ...
    while time.time() - start_time <= t:
        if ngen > 0:
            publish(
                {"label": "Random Search [Process-{}]: NGEN ({})".format(pid, ngen)}, "message")
...
def ea_custom(pid, t, population, toolbox, cxpb, mutpb, publish, res):
    ...
    while time.time() - start_time <= t:
        if ngen > 0:
            publish(
                {"label": "Evolutionary Algorithm [Process-{}]: NGEN ({})".format(pid, ngen)}, "message")
        ...
        offspring = algorithms.varOr(
            population, toolbox, 10, cxpb, mutpb)  # 20 for mu or lambda
        ...
        population[:] = toolbox.select(
            population + offspring, 10)  # Mu plus lambda : ea
