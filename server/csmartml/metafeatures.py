...
class Meta:
    ...
    def meta_features_statistical(self, data, num_cols):
        ...
        if nr_numerical_features != 0:
            ...
            try:
                rho_min, rho_mean, \
                    rho_std, rho_max = self.summary_stats(
    ...
    def all_metafeatures_db(self, data):
        ...
        metafeatures = self.dict_merge(
            metafeatures2, metafeatures3, metafeatures4)
        return metafeatures
        ...
        if missing_val != 0:
            ...
            for index, num_imput_type in enumerate(imputation_types):
                ...
                if meta_type == "attribute":
                    imputed_data[num_cols] = self.numeric_impute(
                        data, num_cols, num_imput_type)
                    #metafeatures1['num_imput_type'] = num_imput_type
                    metafeatures = self.all_metafeatures(
            ...
            if meta_type == "attribute":
                ...
                metafeatures = self.all_metafeatures(
                    data, num_cols, metafeatures1)
                results = pd.DataFrame([metafeatures])
            elif meta_type == "distance":
                metafeatures = self.all_metafeatures_db(
                    self.distance_transform(data))
                results = pd.DataFrame([metafeatures])
    ...
    def extract_for_all(self, path, meta_type):
        ...
        for idx, file in enumerate(allFiles):
            ...
            results = pd.concat(
                [results, self.extract_metafeatures(file, meta_type)], axis=0)
