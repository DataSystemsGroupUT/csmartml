...
def initialize_app():
    ...
    def taskrun():
        ...
        if "algorithm" in task_data.keys():
            ...
            comb = cm.CSmartML(dataset, POP_SIZE, time_budget, publish, False,
                               algorithm=alg, cvi=metric, dataset=uploadedData,
                               result=resultPreference)
            pops, algorithm = comb.search(publish)
        else:
            comb = cm.CSmartML(dataset, POP_SIZE, time_budget, publish,
                               True, dataset=uploadedData,
                               result=resultPreference)
            pops, algorithm = comb.search(publish)
        ...
        return {"clusters": configurations, "pca": pca_2d.tolist(),
                "data": data.to_numpy().tolist(),
                "configs": configuration_values}
